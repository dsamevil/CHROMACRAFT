...
def index():
    if request.method == 'POST':
        ...
        # Call the enhance_image function from your existing code
        enhanced_output_image = enhance_image(img_path, 'static')
...
def clear():
    ...
    for filename in os.listdir(app.config['UPLOAD_FOLDER']):
        if filename != 'placeholder.txt':  # Exclude placeholder file if present
            file_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
            os.remove(file_path)

    # Clear colorized and enhanced images
    for folder in ['colorized', 'enhanced']:
        for filename in os.listdir(os.path.join('static', folder)):
